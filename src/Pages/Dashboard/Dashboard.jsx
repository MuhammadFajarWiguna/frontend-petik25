...
import { Line, LineChart, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip, Legend, BarChart, Bar } from "recharts";
...
const Dashboard = () => {
  ...
  const grafikData = Array.from({ length: 7 }, (_, i) => {
    ...
    const pesananHari = pesanan.filter((p) => p.tanggal === date.toISOString().split("T")[0]);
  ...
  const grafik = Array.from({length: 7}, (_, i) => {
    const date = new Date(today);
    date.setDate(date.getDate() - (6 - i));
    ...
    const jumPesanan = pesanan.filter((p) => p.tanggal === date.toISOString().split("T") [0])

    return {
      tanggal,
      total: jumPesanan.length,
    }
  })
  return (
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={grafikData}>
              <Line
                ...
                dataKey="total"
                stroke="#2d49ff"
                strokeWidth={2} //ini untuk mengatur ketebalan garisnya
                dot={{ r: 2, fill: "#2d49ff" }} //untuk mengatur lingkarannya
              />
              <XAxis dataKey="tgl" />
